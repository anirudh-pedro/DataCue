    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if result.get("success"):
                ...
                print(f"  Skipped: {Colors.YELLOW}{dashboard.get('skipped_charts', 0)}{Colors.RESET}")
                ...
                for i, chart in enumerate(dashboard['charts'], 1):
                    ...
                    if chart['status'] == 'skipped':
                        print(f"    {Colors.YELLOW}Reason: {chart.get('reason', 'Unknown')}{Colors.RESET}")
                    
                    elif chart['status'] == 'success':
                        data = chart['data']
                        ...
                        if 'sql_query' in chart:
                            sql_query = chart['sql_query'].replace('\n', ' ')
...
def main():
    ...
    # Show Phase 2 preview (calls LLM)
    show_phase2_preview(schema)
